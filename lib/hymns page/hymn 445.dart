import 'package:flutter/material.dart';

class Hymn445 extends StatefulWidget {
  const Hymn445({super.key});

  @override
  State<Hymn445> createState() => _Hymn445State();
}

class _Hymn445State extends State<Hymn445> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.blue,
          title: Row(
            mainAxisAlignment: MainAxisAlignment.start,
            children: [
              Text(
                "Seraph Hymns\nOrin mimo kerubu ati serafu",
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 20,
                    fontWeight: FontWeight.normal
                ),
              ),
            ],
          ),
          actions:[
            Text(
              "K&S 445", // Corrected Hymn Number
              style: TextStyle(
                  color: Colors.red,
                  fontSize: 35,
                  fontWeight: FontWeight.bold
              ),
            ),
          ]
      ),
      body: SingleChildScrollView(
        child: Column(
          children: [
            Container(
              decoration: BoxDecoration(
                borderRadius: BorderRadius.all(Radius.circular(40)),
                color: Colors.transparent,
              ),
              child: Center(
                child: Column(
                  children: [
                    Padding(
                      padding: const EdgeInsets.all(15.0),
                      child: Container(
                        decoration: BoxDecoration(
                            color: Colors.transparent,
                            borderRadius: BorderRadius.circular(30)
                        ),
                        child: Center(
                          child: ListTile(
                            title: Text('445 C.M.S. 181 t.H.C. 153. 8s. (FE 469)', // Corrected Title
                              style: TextStyle(color: Colors.black,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 22),
                            ),
                            subtitle: Text( // Corrected Subtitle
                              '“Tani Ọlọrun bi Iwọ, ti o n dari aisedede ji.” - Mik. 7:18',
                              style: TextStyle(color: Colors.red,
                                  fontWeight: FontWeight.w700,
                                  fontSize: 18),
                            ),
                          ),
                        ),
                      ),
                    ),
                  ],
                ),
              ),
            ),
            // Verse 1
            Center(
              child: Column(
                children: [
                  Padding(
                    padding: const EdgeInsets.all(15.0),
                    child: Text("1.mf	    Ọlọrun 'yanu ọna kan,\n" // Corrected Verse
                        "Ti o da bi Tirẹ ko si:\n"
                        "Gbogb' ogo ore-ọfẹ Rẹ,\n"
                        "L'o farahan bi Ọlọrun,\n"
                        "ff   		  Egbe:	    Ta l'Ọlọrun ti n dariji,\n"
                        "Ore ta l'o pọ bi Tirẹ?",
                      style: TextStyle(color: Colors.black,
                          fontWeight: FontWeight.normal,
                          fontSize: 22),
                    ),
                  ),
                ],
              ),
            ),
            // Verse 2
            Center(
              child: Column(
                children: [
                  Padding(
                    padding: const EdgeInsets.all(15.0),
                    child: Text("2.		    N' iyanu at'ayọ l'a gba,\n" // Corrected Verse
                        "Idariji Ọlọrun wa;\n"
                        "p	    'Dariji f'ẹsẹ t'o tobi,\n"
                        "T'a f'ẹjẹ Jesu s'edidi.\n"
                        "ff   		  Egbe:	    Ta l'Ọlọrun ti n dariji,\n"
                        "Ore ta l'o pọ bi Tirẹ?",
                      style: TextStyle(color: Colors.black,
                          fontWeight: FontWeight.normal,
                          fontSize: 22),
                    ),
                  ),
                ],
              ),
            ),
            // Verse 3
            Center(
              child: Column(
                children: [
                  Padding(
                    padding: const EdgeInsets.all(15.0),
                    child: Text("3.		    Jẹ ki ore-ọfẹ Rẹ yi,\n" // Corrected Verse
                        "Ifẹ iyanu nla Rẹ yi,\n"
                        "K'o f'iyin kun gbogbo ayé,\n"
                        "Pẹlu ẹgbẹ Angel l'oke\n"
                        "ff   		  Egbe:	    Ta l'Ọlọrun ti n dariji,\n"
                        "Ore ta l'o pọ bi Tirẹ?",
                      style: TextStyle(color: Colors.black,
                          fontWeight: FontWeight.normal,
                          fontSize: 22),
                    ),
                  ),
                ],
              ),
            ),
            // Amin
            Column(
              children: [
                Text(
                  "\nAMIN",
                  style: TextStyle(color: Colors.red,
                      fontWeight: FontWeight.bold,
                      fontSize: 22),
                ),
                SizedBox(height: 50,)
              ],
            ),
          ],
        ),
      ),
    );
  }
}
